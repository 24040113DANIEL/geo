// ====== 1) MAPA BASE ======
const map = L.map("map").setView([25.4232, -101.0053], 13); // Saltillo (ejemplo)
...
// Catálogo de ejemplos (para “mostrar diferentes tipos de ubicación”)
const ejemplos = {
  coordenadas: {
...
async function geocodeWithNominatim(query) {
  // API pública de OSM (sin key). Recomendación: usar con moderación.
  ...
  try {
    if (tipo === "usuario") {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          info.textContent =
            "No se pudo obtener tu ubicación. Asegúrate de permitir el acceso al GPS en el navegador.\n" +
            `Error: ${err.message}`;
        },
    ...
    // Dirección / Ciudad / POI => geocoding
